...
class SQLiteDataLayer(BaseDataLayer):
    ...
    async def get_thread(self, thread_id: str) -> Optional[ThreadDict]:
        ...
        if user_threads:
            print('get_thread', user_threads[0]["steps"])
        ...
        if isinstance(steps_feedbacks, list):
            for step_feedback in steps_feedbacks:
                ...
                if thread_id is not None:
                    ...
                    if isinstance(step_feedback['step_streaming'], int):
                        step_feedback['step_streaming'] = False
                    if isinstance(step_feedback['step_waitforanswer'], int):
                        step_feedback['step_waitforanswer'] = False
                    if isinstance(step_feedback['step_iserror'], int):
                        step_feedback['step_iserror'] = False
                    step_dict = StepDict(
