# 请将该脚本覆盖 venv/lib/site-packages/vanna/base/base.py
...
class VannaBase(ABC):
    ...
    def get_training_plan_mysql(self, df) -> TrainingPlan:
        """
        add by tianweifeng 2024-12-28
        ...
        for database in df[database_column].unique().tolist():
            for schema in (
                ...
                for table in (
                    ...
                    doc = f"The following columns are in the {table} table in the {schema} database:\n\n"
                    doc += df_columns_filtered_to_table[columns].to_markdown()
